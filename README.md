...
cargo run -p mls_pqc_cli -- join-group "my-group" -m "Bob" --welcome .mls_state/my-group/artifacts/welcome/<timestamp>_bob_kp.bin --key-package-data bob_kp_data.json
```
...
```powershell
# Decrypt received ciphertext (via direct file path - recommended!)
cargo run -p mls_pqc_cli -- decrypt -g "my-group" --ciphertext-file .mls_state/my-group/artifacts/ciphertext/<timestamp>_000000.bin

# Alternatively, copy-paste raw Base64 string
...
cargo run -p mls_pqc_cli -- join-group "project-alpha" -m "Bob" --welcome $welcome.FullName --key-package-data bob_keypackage_data.json
...
# ============================================
# First, fetch the exported ciphertext artifact generated by Alice
$ct = Get-ChildItem .mls_state/project-alpha/artifacts/ciphertext/*.bin | Select-Object -First 1

# Use Bob's state file (project-alpha_Bob.json) and read exactly from disk
cargo run -p mls_pqc_cli -- -d .mls_state decrypt -g "project-alpha_Bob" --ciphertext-file $ct.FullName
...
The CLI outputs JSONL with timing and size metrics suitable for benchmarking and research. We provide automated scripts for running benchmarks against all cryptographic suites and generating performance visualizations.

### Automated Benchmarking Pipeline

Run the automated benchmark suite to collect metrics:

```bash
# Run the end-to-end benchmark for 'classic', 'pqc-kem', and 'hybrid-kem' suites
./run_benchmarks.sh
```

This will run group initialization, member operations, encryption, and decryption, resulting in a benchmark dataset stored in `results.jsonl`.

### Performance Visualizations

After generating `results.jsonl`, run the Python plotting script to visualize the security vs performance tradeoffs:

```bash
# Install requirements if needed
pip install pandas matplotlib

# Generate comparative PNG charts
python plot_benchmarks.py
```

### Example Benchmark Visualizations

![Encryption Time](encrypt_time.png)

![Decryption Time](decrypt_time.png)

![Ciphertext Size](ciphertext_size.png)
...
## 🔭 Quantum Threat Showcase & Executive Dashboard

To effectively communicate *why* this PQC-Enhanced MLS architecture is mandatory for modern cryptographic security, we have built a standalone **Quantum Threat Showcase** located in `bench/quantum_showcase/`.

### Live Shor's Algorithm Execution
The showcase includes a direct integration with **IBM Quantum** (via Qiskit). It actively runs a simulated (or real hardware) period-finding circuit—the core computational sub-routine of Shor's Algorithm—proving natively how easily a quantum interference circuit collapses the prime factorization paradigm required by classical RSA/ECC encryption schemas. 

### Visual Security Dashboard
By synthesizing the physical quantum probability logs together with our PQC MLS backend telemetry, we provide a unified **2x2 Executive Visual Dashboard** (`plot_dashboard.py`).
This graphic visualizes:
1. **Quantum Threat Matrix**: Evaluating categorical component survival.
2. **Key & Ciphertext Trace**: Demonstrating the bandwidth tradeoffs for ML-KEM.
3. **Qiskit Measurement Output**: The true quantum state distribution forcing classical failure.
4. **Security Bits Timeline**: Showing the degradation of security bits through Grover's and Shor's paradigms.

**To run the showcase:**
```powershell
cd bench/quantum_showcase
python run_showcase.py       # Execute quantum circuit
python plot_dashboard.py     # Generate executive dashboard
```

See the [Quantum Showcase README](bench/quantum_showcase/README_SHOWCASE.md) for deep-dive research regarding the Hybrid > PQC transition timeline.
